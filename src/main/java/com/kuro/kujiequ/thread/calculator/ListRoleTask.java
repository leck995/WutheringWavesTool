 ...
 */
public class ListRoleTask extends Task<ResponseBody<RoleForCalculator>> {
    ...
    @Override
    protected ResponseBody<RoleForCalculator> call() throws Exception {
        return request(signUserInfo.getToken());
    }

    private ResponseBody<RoleForCalculator> request(String token){
        ...
        try {
            ...
            if (response.statusCode() == 200) {
                ...
                ResponseBody<RoleForCalculator> responseBody = mapper.readValue(response.body(), new TypeReference<ResponseBody<RoleForCalculator>>() {
                });
                return responseBody;
            }else {
