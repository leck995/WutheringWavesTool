...
public final class ResourceHelper {
    ...
    public static void updateResultInfoDownloadDiskSizeInfo(
            UpdateResult updateResult,
            ResourceConfigManager configManager,
            cn.tealc.wwt.game.resource.internal.legacy.download.ResourcesDownloadTask resourcesDownloadTask,
            long unCompressSize) {
    ...
    public static void getFileIndexInfo(String cacheIndexFilePath, String url,
        ...
        try {
            ...
            cn.tealc.wwt.game.resource.internal.legacy.download.DownloadTaskBuilder builder = new cn.tealc.wwt.game.resource.internal.legacy.download.DownloadTaskBuilder(url,
                    indexFilePath);
            builder.withMd5(md5);
            ...
            cn.tealc.wwt.game.resource.internal.legacy.download.DownloadTask task = builder.build();

            task.setStateCallback((state, error) -> {
                if (state == cn.tealc.wwt.game.resource.internal.legacy.download.DownloadState.COMPLETE) {
                    parseFileIndexInfo(indexFilePath, callback, applyType);
                } else if (state == cn.tealc.wwt.game.resource.internal.legacy.download.DownloadState.FAILED
                        || state == cn.tealc.wwt.game.resource.internal.legacy.download.DownloadState.CANCELED) {
